...
Page({
  ...
  data: {
    ...
    actionSheetItems: [
      ...
      { bindtap: 'RejectService', txt: '服务驳回' },
    ],
    requestAddress: '',
    modalHidden: true,
    modalTitle: '',
  ...
  bindRejectService: function () {
    if (this.data.faultInfo.faultFrom == '7' || this.data.faultInfo.faultFrom == '8') {
      this.setData({
        popErrorMsg: '点检或保养工单不能驳回'
      });
      this.ohShitfadeOut();
    } else {
      this.setData({
  ...
  rejectModalChange: function () {
    ...
    var param = {
      ...
      'type': '2',
      'processId': this.data.faultInfo.processId,
      'processPerId': this.data.faultInfo.perId,
    }
