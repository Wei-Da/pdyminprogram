...
Page({
  ...
  fetchServiceData: function () {  //获取服务数据
    ...
    if ('3' == wx.getStorageSync('appRole')){
      ...
      param['changeInfo.changeUserid'] = wx.getStorageSync("userInfo").id;
    }
    console.log(wx.getStorageSync('userInfo').id)
    app.webCall(app.serviceCode["WAIT_SERVICE_CHANGE"], param,
  ...
  keyWordsSearch: function () {
    wx.navigateTo({
      url: "../../keySearch/keySearch?path=serviceChange&&param=faultInfo.sysuserid&&mUrl=SERVICE_DEALING&&type='1'&&role=1",
    })
  },
