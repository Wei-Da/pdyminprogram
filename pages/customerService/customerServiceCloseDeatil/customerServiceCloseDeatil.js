...
Page({
  ...
  data: {
    ...
    actionSheetItems: [
      { bindtap: 'ConfirmService', txt: '服务客评' }, 
    ],
        ...
        for (var j = 0; j < res.listTimeRecord.length; j++) {
          ...
          if (tempSteps.length == 2 && tempSteps[0].stage == '1' && tempSteps[1].stage == '7') {
            for (var i = 0; i < that.data.defaultSteps.length; i++) {
              ...
              if (i == 0) {
                tempSteps[i] = { dualdate: tempSteps[0].dualdate };
              }
              if (i == 4) {
                tempSteps[i] = { dualdate: tempSteps[1].dualdate };
              }
            }
            ...
            for (var i = 0; i < that.data.defaultSteps.length; i++) {
              ...
              if (i == 5) {
                tempSteps[i] = { dualdate: tempSteps[2].dualdate };
              }
            }
          }else{
  ...
  bindConfirmService: function () {
    if (wx.getStorageSync('userInfo').id == this.data.faultInfo.sysuserId){
          wx.redirectTo({
            url: '../../customerService/customerEvaluate/customerEvaluate?faultId=' + this.data.faultId,
          })
    }else{
      this.setData({
        popErrorMsg: '您不是工单报修人，无法客评',
      })
      this.ohShitfadeOut();
      return;
    }
